...
* `coframe`: Coframe: A complete lattice whose `↑⊔` distributes over `⨅`.
...
/-- A coframe, aka complete co-Heyting algebra, is a complete lattice whose `⊔` distributes over
`⨅`. -/
class coframe (α : Type*) extends complete_lattice α :=
...
instance : coframe (order_dual α) :=
{ infi_sup_le_sup_Inf := frame.inf_Sup_le_supr_inf,
...
instance : frame (order_dual α) :=
{ inf_Sup_le_supr_inf := coframe.infi_sup_le_sup_Inf,
  .. order_dual.complete_lattice α }
